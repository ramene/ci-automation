...
ssh -i pcf.pem -o StrictHostKeyChecking=no ubuntu@${OPSMAN_HOST} "mysql -h $db_host -u $db_username -p$RDS_PASSWORD < /tmp/databases.sql"
...
sed -i -e "s/{{aws_zone_1}}/${az1}/g" ${json_file}
sed -i -e "s/{{aws_zone_2}}/${az2}/g" ${json_file}
sed -i -e "s/{{aws_zone_3}}/${az3}/g" ${json_file}
sed -i -e "s/{{rds_host}}/${db_host}/g" ${json_file}
sed -i -e "s/{{rds_user}}/${db_username}/g" ${json_file}
sed -i -e "s/{{rds_password}}/${RDS_PASSWORD}/g" ${json_file}
sed -i -e "s/{{pcf_ert_domain}}/${ERT_DOMAIN}/g" ${json_file}
sed -i -e "s/{{pcf_environment}}/${environment}/g" ${json_file}
sed -i -e "s/{{aws_access_key}}/${AWS_ACCESS_KEY_ID}/g" ${json_file}
sed -i -e "s/{{aws_secret_key}}/${AWS_SECRET_ACCESS_KEY}/g" ${json_file}
sed -i -e "s/{{aws_region}}/${region}/g" ${json_file}
sed -i -e "s#{{s3_endpoint}}#${S3_ENDPOINT}#g" ${json_file}
