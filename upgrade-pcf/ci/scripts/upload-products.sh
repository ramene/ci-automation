...
om-alpine -t https://$OPSMAN_HOST -k -c "$OPSMAN_USER" -s "$OPSMAN_PASSWORD" delete-unused-products
...
if [[ $stemcellCount != 0 ]]; then
    for file in pcf-products/*.tgz; do
        ...
        om-alpine -t https://$OPSMAN_HOST -k -c "$OPSMAN_USER" -s "$OPSMAN_PASSWORD" upload-stemcell --stemcell $file
...
if [[ $productCount != 0 ]]; then
    for file in pcf-products/*.pivotal; do
        ...
        om-alpine -t https://$OPSMAN_HOST -k -c "$OPSMAN_USER" -s "$OPSMAN_PASSWORD" upload-product --product $file
